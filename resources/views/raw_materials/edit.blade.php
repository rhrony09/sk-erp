{{ Form::model($raw_material, ['route' => ['productservice.raw_material.update', $raw_material->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data']) }}
<div class="modal-body">
    ...
    <div class="row">
        ...
        <div class="form-group col-md-6">
            ...
            <select name="sale_chartaccount_id" class="form-control" required="required">
                @foreach ($incomeChartAccounts as $key => $chartAccount)
                    <option value="{{ $key }}" class="subAccount" {{ $raw_material->sale_chartaccount_id == $key ? 'selected' : '' }}>{{ $chartAccount }}</option>
                    @foreach ($incomeSubAccounts as $subAccount)
                        @if ($key == $subAccount['account'])
                            <option value="{{ $subAccount['id'] }}" class="ms-5" {{ $raw_material->sale_chartaccount_id == $subAccount['id'] ? 'selected' : '' }}> &nbsp; &nbsp;&nbsp; {{ $subAccount['code_name'] }}</option>
                        @endif
                    @endforeach
                @endforeach
            </select>
        ...
        <div class="form-group col-md-6">
            ...
            <select name="expense_chartaccount_id" class="form-control" required="required">
                @foreach ($expenseChartAccounts as $key => $chartAccount)
                    <option value="{{ $key }}" class="subAccount" {{ $raw_material->expense_chartaccount_id == $key ? 'selected' : '' }}>{{ $chartAccount }}</option>
                    @foreach ($expenseSubAccounts as $subAccount)
                        @if ($key == $subAccount['account'])
                            <option value="{{ $subAccount['id'] }}" class="ms-5" {{ $raw_material->expense_chartaccount_id == $subAccount['id'] ? 'selected' : '' }}> &nbsp; &nbsp;&nbsp; {{ $subAccount['code_name'] }}</option>
                        @endif
                    @endforeach
                @endforeach
            </select>
        ...
        <div class="col-md-6 form-group">
            ...
            <div class="choose-file ">
                <label for="pro_image" class="form-label">
                    ...
                    <img id="image" class="mt-3" width="100" src="@if ($raw_material->pro_image) {{ asset(Storage::url('uploads/pro_image/' . $raw_material->pro_image)) }}@else{{ asset(Storage::url('uploads/pro_image/user-2_1654779769.jpg')) }} @endif" />
                </label>
            </div>
        </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-check form-check-inline">
                            <input type="radio" class="form-check-input type" id="customRadio5" name="type" value="product" @if ($raw_material->type == 'product') checked @endif>
                            <label class="custom-control-label form-label" for="customRadio5">{{ __('Product') }}</label>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-check form-check-inline">
                            <input type="radio" class="form-check-input type" id="customRadio6" name="type" value="raw_material" @if ($raw_material->type == 'raw_material') checked @endif>
                            <label class="custom-control-label form-label" for="customRadio6">{{ __('Raw Material') }}</label>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="form-group col-md-6 quantity {{ $raw_material->type == 'service' ? 'd-none' : '' }}">
