    ...
    public function index()
    {
        if(\Auth::user()->can('manage appraisal'))
        {
            ...
            if($user->type == 'Employee')
            {
                ...
                $appraisals = Appraisal::where('branch', $employee->branch_id)->where('employee', $employee->id)->with(['branches','employees'])->get();
            }
            else
            {
                $competencyCount = Competencies::where('created_by', '=', $user->creatorId())->count();

                $appraisals = Appraisal::with(['branches','employees'])->get();
            }
