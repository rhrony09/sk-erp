...
class EmployeeController extends Controller
{
    ...
    ->first();
        ...
        if (\Auth::user()->can('manage employee')) {
            if (Auth::user()->employee && $role && $role->role_id != 10) {
                $employees = Employee::where('branch_id', '=', Auth::user()->employee->branch_id)->with(['branch', 'department', 'designation'])->get();
            } else {
                $employees = Employee::with(['branch', 'department', 'designation'])->get();
            }
