    ...
    public function account_dashboard_index() {
        ...
        if (Auth::check()) {
            if (Auth::user()->type == 'client') {
                ...
                {
                    $data['latestIncome']  = Revenue::with(['customer'])->orderBy('id', 'desc')->limit(5)->get();
                    $data['latestExpense'] = Payment::with(['vender'])->orderBy('id', 'desc')->limit(5)->get();


                    $incomeCategory = ProductServiceCategory::where('type', '=', 'income')->get();
                    $inColor        = array();
                    $inCategory     = array();
                    $inAmount       = array();
                    ...
                    $expenseCategory = ProductServiceCategory::where('type', '=', 'expense')->get();
                    $exColor         = array();
                    $exCategory      = array();
                    $exAmount        = array();
                    ...
                    $data['bankAccountDetail'] = BankAccount::get();
                    ...
                    $data['orders']            = ProductServiceOrder::where('status', '!=','delivered')->orWhere('status', '!=', 'cancelled')->orderBy('created_at', 'asc')->get();
                    $data['productStocks']     = WarehouseProduct::where('quantity', '<=', 5)->with(['stockProduct', 'warehouse'])->get();
    ...
    public function crm_dashboard_index() {
        ...
        if (\Auth::user()->can('show crm dashboard')) {
            if ($user->type == 'admin') {
                ...
                $crm_data['latestContract']  = Contract::orderBy('id', 'desc')->limit(5)->with(['clients', 'projects', 'types'])->get();
