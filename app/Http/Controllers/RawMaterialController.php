...
use App\Models\WarehouseRawMaterial;
...
class RawMaterialController extends Controller {
    ...
    public function raw_material_index(Request $request) {
        ...
        if (\Auth::user()->can('manage product & service')) {
            ...
            if (!empty($request->category)) {
                ...
                $raw_materials = RawMaterial::where('category_id', $request->category)->with(['taxes', 'unit', 'category'])->get();
            } else {
                $raw_materials = RawMaterial::with(['taxes', 'unit', 'category'])->get();
            }

            return view('raw_materials.index', compact('raw_materials', 'category'));
        } else {
    ...
    public function store(Request $request) {
        ...
        if (\Auth::user()->can('create product & service')) {
            ...
            $rules = [
                'name' => 'required',
                'sku' => [
                    'required', Rule::unique('raw_materials')->where(function ($query) {
            ...
            if ($request->type == 'raw_material') {
                ...
                return redirect()->route('productservice.index')->with('success', __('Raw Material successfully created.'));
            } else {
    ...
    public function edit($id) {
        ...
        if (\Auth::user()->can('edit product & service')) {
            if ($raw_material->created_by == \Auth::user()->creatorId()) {
                $category = ProductServiceCategory::where('type', '=', 'product & service')->get()->pluck('name', 'id');
                $unit     = ProductServiceUnit::get()->pluck('name', 'id');
                $tax      = Tax::get()->pluck('name', 'id');

                $raw_material->customField = CustomField::getData($raw_material, 'product');
                $customFields                = CustomField::where('module', '=', 'product')->get();
                $raw_material->tax_id      = explode(',', $raw_material->tax_id);
                $incomeChartAccounts = ChartOfAccount::select(\DB::raw('CONCAT(chart_of_accounts.code, " - ", chart_of_accounts.name) AS code_name, chart_of_accounts.id as id'))
                    ->leftjoin('chart_of_account_types', 'chart_of_account_types.id', 'chart_of_accounts.type')
                    ->where('chart_of_account_types.name', 'income')
                    ->where('parent', '=', 0)
                ...
                return view('raw_materials.edit', compact('category', 'unit', 'tax', 'raw_material', 'customFields', 'incomeChartAccounts', 'expenseChartAccounts', 'incomeSubAccounts', 'expenseSubAccounts'));
            } else {
                return response()->json(['error' => __('Permission denied.')], 401);
            }
        } else {
    ...
    public function update(Request $request, $id) {
        ...
        if (\Auth::user()->can('edit product & service')) {
            ...
            if ($raw_material->created_by == \Auth::user()->creatorId()) {
                $rules = [
                    'name' => 'required',
                    'sku' => 'required', Rule::unique('raw_materials')->ignore($raw_material->id),
                    'sale_price' => 'required|numeric',
                    'purchase_price' => 'required|numeric',
                    'category_id' => 'required',
                    'unit_id' => 'required',
                ...
                if ($validator->fails()) {
                    ...
                    return redirect()->route('productservice.raw_material.index')->with('error', $messages->first());
                }
                ...
                return redirect()->route('productservice.raw_material.index')->with('success', __('Raw Material successfully updated.'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function destroy($id) {
        if (\Auth::user()->can('delete product & service')) {
            ...
            if ($raw_material->created_by == \Auth::user()->creatorId()) {
                ...
                return redirect()->route('productservice.raw_material.index')->with('success', __('Raw Material successfully deleted.'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function warehouseDetail($id) {
        $products = WarehouseRawMaterial::with(['warehouse'])->where('raw_material_id', '=', $id)->get();
        return view('raw_materials.detail', compact('products'));
    }
