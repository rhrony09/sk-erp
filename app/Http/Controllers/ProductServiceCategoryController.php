    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create constant category'))
        {
            ...
            if (!empty($request->cat_image)) {
                ...
                $fileName = $request->cat_image->getClientOriginalName();
                $dir        = 'uploads/cat_image';
                $path = Utility::upload_file($request, 'cat_image', $fileName, $dir, []);
                $category->thumbnail = $dir.'/'.$fileName;
            }
            $category->save();
        ...
        if(\Auth::user()->can('edit constant category'))
        {
            ...
            if($category->created_by == \Auth::user()->creatorId())
            {
                $validator = \Validator::make(
                    $request->all(), [
                        'name' => 'required|max:200',
                        'slug' => 'unique:product_service_categories,slug',
                        'type' => 'required',
                        'color' => 'required',
                    ]
                );
                ...
                if (!empty($request->cat_image)) {
                    ...
                    if ($category->thumbnail) {
                        $path = storage_path( $category->thumbnail);
                        if (file_exists($path)) {
                            \File::delete($path);
                        }
                    }
